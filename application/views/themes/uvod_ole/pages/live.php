...
?>
    ...
    function channel_change(release_url, channel_obj, id, channel_name) {
                ...
                TweenMax.to(channel_obj, .3, {boxShadow: "2px 0px 20px rgba(115,189,238,0.8)", border: "1px solid rgba(115,189,238,0.8)"});
                    ...
                    <div id="live_channels_scroller">
                        ...
                        if ($channels && isset($channels->entries)) {
                            for ($i = 0; $i < sizeof($channels->entries); $i++) {
                                ...
                                ?>
                                <div class="live_channel_container" station="<?php echo $channels->entries[$i]->id; ?>" id="channel_<?php echo $i; ?>" <?php echo 'style="background: url(' . $channel_image . ') center center no-repeat; background-size: 126px 68px;' . ($i == 0 ? 'box-shadow: 2px 0px 20px rgba(115,189,238,0.8); border: 1px solid rgba(115,189,238,0.8);"' : '"'); ?>></div>
                                <?php
                            }
                        }
                        ?>
