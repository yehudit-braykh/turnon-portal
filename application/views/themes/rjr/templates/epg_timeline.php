...
//                        }
//                    } else {
//                        if (isset($epg->content->entries[$i]->{'program'}->{'plprogram$runtime'}) &&
//                                intval($epg->content->entries[$i]->{'program'}->{'plprogram$runtime'}) != 0 &&
//                                sizeof($title_arr) > 1) {
//
//                            $title = $title_arr[1];
//                        } else {
                ...
                if ($today == $currentDate) {
                    ...
                    if ($country == 'Jamaica') {
                        ...
                        if ($title_arr[0] != '') {
                            $title = $title_arr[0];
                            if ($i == 0) {
                                echo '<tr class="current_row"><td class="epgTime current_epg_date">' . date('h:i A', ($epg->content->entries[$i]->{'pllisting$startTime'} / 1000)) . '</td>'
                                . '<td class="epgData epgDataCurrent" style="border-right:8px solid #7f00bf;">' . '<span>' . $title . '</span></td></tr>';
                            } else {
                                echo '<tr><td class="epgTime">' . date('h:i A', ($epg->content->entries[$i]->{'pllisting$startTime'} / 1000)) . '</td>'
                                . '<td class="epgData" id="' . $epg->content->entries[$i]->{'pllisting$startTime'} . '">' . '<span>' . $title . '</span></td></tr>';
                            }
                 
                        }
                    } else {
                        if (isset($epg->content->entries[$i]->{'pllisting$program'}->{'plprogram$runtime'}) &&
                                intval($epg->content->entries[$i]->{'pllisting$program'}->{'plprogram$runtime'}) != 0 &&
                                sizeof($title_arr) > 1) {
