              ...
              <?php
                ...
                if ($this->config->item('show_logos_for_vod_categories') !== FALSE) {
                  ...
                  for($i=0;$i<sizeof($vod_categories);$i++) { 
                    echo '<div id="top_sub_menu_' . $vod_categories[$i]->id . '" style="margin-right:10px; height:21px;" ' . ($sub_section1 == $vod_categories[$i]->id ? 'class="top_sub_menu_selected"' : '') . '>' . $vod_categories[$i]->title . '</div>';
                  }
                  echo '</div>';
