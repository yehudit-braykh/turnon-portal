$(document).ready(function () {
    ...
    $(document).on('click', '.select_day', function () {
        ...
        for (i = 0; i < contentEpg.length; i++) {
            ...
            programDate = get_date(contentEpg[i].pllisting$startTime);

            if (selectDate == programDate) {

                hour = get_hour(contentEpg[i].pllisting$startTime);
                title_arr = contentEpg[i].pllisting$program.pl$title.split('|');
                ...
                if (country_var == 'Jamaica') {
                    ...
                    if (typeof contentEpg[i].pllisting$program.plprogram$runtime != 'undefined' &&
                            contentEpg[i].pllisting$program.plprogram$runtime != 0 && title_arr.length > 1) {
                        title = title_arr[1];
                    } else {
                        title = title_arr[0];
                    }
