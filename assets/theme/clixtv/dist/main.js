...
angular.module('clixtv').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui/categories/view.category.html',
    "<div ng-if=!category><clix-loader size=large></clix-loader></div><div class=clix-category-page ng-if=category><clix-hero-banner title-text={{category.title}} button-text=\"{{'Favorites'}}\" subtext=\"{{category.videos.videos.length}} {{category.videos.videos.length === 1 ? 'Video' : 'Videos'}}\" button-icon-class=\"{{'icon-favorite-icon banner-favorite-icon'}}\" background-image={{category.headerImage}} background-image2x={{category.headerImage}} background-image3x={{category.headerImage}} shareable=false button-tooltip-text=\"{{loggedInUser ? (isFavorite ? 'Remove this category from your Favorites' : 'Add this category to your Favorites.') : 'After signing up, you will be able to add this category to your Favorites.'}}\" share-tooltip-text=\"Share this category.\" banner-type=Category><favorite-button><clix-tertiary-button ng-click=onFavoritePress() is-active=isFavorite>{{isFavorite ? 'Remove from Favorites' : 'Favorite'}}</clix-tertiary-button></favorite-button></clix-hero-banner><div class=category-page-container><div class=category-page-content><div class=\"category-list-container hidden-sm hidden-xs\"><div class=category-list-content><div class=category-list-title>More Categories</div><ul class=category-list><li ng-repeat=\"relatedCategory in categories.categories | orderBy: 'order'\" class=category-list-item ng-class=\"{'active-category': relatedCategory.title === category.title}\"><a ui-sref=\"category({ id: '{{relatedCategory.id}}' })\">{{relatedCategory.title}}</a></li></ul></div></div><div class=category-content><div class=category-filter-bar><div class=\"category-filter-container row\" ng-class=\"{'hidden-category-filters': (!category.videos.videos || category.videos.videos.length === 0)}\"><div class=\"filter-container col-xs-6\"><clix-dropdown options=filterOptions placeholder-text=\"Filter By\"></clix-dropdown></div><div class=\"filter-container col-xs-6\"><clix-dropdown options=sortOptions placeholder-text=\"Sort By\"></clix-dropdown></div></div></div><div class=empty-videos ng-if=\"!category.videos || !category.videos.videos || category.videos.videos.length === 0\"><div class=empty-category-container><div class=empty-category-title>{{category.title}} videos are coming soon!</div><clix-is-logged-in><logged-in><div class=empty-category-desc>ClixTV can let you know when new<br>content is uploaded into this category!</div><div class=empty-category-notify-container><div class=empty-category-notify><div class=empty-category-notify-label>Notify Me!</div><div class=empty-category-switch><clix-switch ng-model=notify></clix-switch></div></div></div></logged-in><not-logged-in><div class=empty-category-desc>Sign up now to make sure you are<br>notified when videos become available!</div></not-logged-in></clix-is-logged-in></div></div><div class=\"row clix-block-row\"><div class=\"clix-block-item col-xs-12 col-sm-12 col-lg-3\" ng-repeat=\"video in category.videos.videos\"><clix-video-content-box video=video></clix-video-content-box></div></div></div></div></div></div>"
  );
